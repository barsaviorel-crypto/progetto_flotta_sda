...
class SistemaGestionaleFlotta:
    def __init__(self, root):
        ...
        # Inizializzazione DataFrame vuoti per evitare errori di riferimento
        self.df_driver = pd.DataFrame(columns=self.campi_d)
        self.df_veicoli = pd.DataFrame(columns=self.campi_v[:-1])

        if self.verifica_licenza():
            self.carica_dati_iniziali()  # Carica i dati dal file JSON se esiste
            self.root.protocol("WM_DELETE_WINDOW", self.chiedi_conferma_uscita)
            self.gestisci_accesso()
        else:
            self.root.destroy()

    def verifica_licenza(self):
        """Verifica se il file license.key contiene l'HWID di questo PC criptato."""
        id_attuale = recupera_hwid()

        if not os.path.exists(self.file_licenza):
            messagebox.showerror("Licenza", f"Licenza mancante.\nInvia questo ID allo sviluppatore(Viorel):\n\n{id_attuale}")
        ...
        try:
            with open(self.file_licenza, "r") as f:
                chiave_criptata = f.read().strip()

            # Decodifica Base64 (compatibile con sblocca.py)
            id_autorizzato = base64.b64decode(chiave_criptata.encode()).decode()

            if id_autorizzato == id_attuale:
                return True
            else:
                messagebox.showerror("Errore Licenza", "Questa copia del software non è autorizzata per questo PC.")
                return False
        except Exception:
            messagebox.showerror("Errore", "File di licenza non valido o corrotto.")
            return False
