...
def recupera_hwid():
    """Recupera l'UUID tramite PowerShell se WMIC non è disponibile."""
    try:
        # Comando PowerShell per ottenere l'UUID
        ...
        # Se l'UUID è vuoto o fallisce, proviamo il Serial Number del BIOS
...
# Sostituisci questa stringa con l'UUID del tuo PC
HWID_AUTORIZZATO = "4C4C4544-0032-5710-8054-B2C04F533734"
# Impostazioni Tema
...
class SistemaGestionaleFlotta:
    ...
    # --- LOGICA DI USCITA E SALVATAGGIO ---
    ...
    def crea_treeview(self, parent, colonne):
        ...
        tree.tag_configure('scaduto', background='#721c24', foreground="white") # Rosso (scaduto)
        tree.tag_configure('avviso', background='#f1c40f', foreground="black")  # Giallo (Scade entro 30gg)
        return tree
    ...
    def gestisci_accesso(self):
        # --- BLOCCO HARDWARE ---
        id_attuale = recupera_hwid()

        if id_attuale != HWID_AUTORIZZATO:
            # Opzionale: mostra l'ID corrente per poterlo copiare la prima volta
            messagebox.showerror("Licenza Non Valida",
                                 f"Software non autorizzato su questo PC.\nID rilevato: {id_attuale}")
            self.root.destroy()
            return
        # ------------------------
            ...
            if data_ins and data_ins.strip():
                try:
                    ...
                    if giorni_mancanti < 0:
                        tag = ('scaduto',)  # Rosso se già passata
                    elif giorni_mancanti <= 30: # Giallo se scade entro 30 giorni
                        tag = ('avviso',)
                except ValueError:
                    pass  # Se la data è scritta male non colora nulla
            # ----------------------------------
    ...
    def popup_driver(self, dati=None):
        ...
        def salva():
            nome, cognome, giro = loc_vars["Nome"].get().strip(), loc_vars["Cognome"].get().strip(), loc_vars[
                "Giro"].get().strip()
            # --- CONTROLLO DUPLICATI ---
            if not dati: # Solo se è un nuovo inserimento
    ...
    def popup_veicolo(self, dati=None):
        ...
        def salva():
            ...
            if not dati and t in self.df_veicoli['Targa'].values:
                messagebox.showerror("Errore", "Questa targa è già presente nel parco veicoli!", parent=win);
                return
