    ...
    if os.path.exists("license.key"):
        ...
        try:
            ...
            if id_autorizzato == id_attuale:
                #messagebox.showinfo("Licenza", "Licenza rilevata. Accesso completo")
            ...
            messagebox.showerror("Errore", f"File licenza corrotto. \nInvia questo ID allo sviluppatore(Viorel):\n\n{id_attuale}")
            sys.exit()
...
class DatabaseSQL:
    ...
    def setup(self):
        # Forza la creazione della tabella veicoli con 6 colonne reali
        ...
        # Aggiornamento automatico della tabella per i vecchi database senza colonna note
        try:
...
class SistemaGestionaleFlotta:
    ...
    def setup_tab_anagrafica(self, perm):
        ...
        # Doppio click/Invio ora apre il NUOVO popup dettagli
    ...
    def aggiorna_dati(self):
        ...
        for v in self.db.cursor.fetchall():
            ...
            # Struttura fissa ordinata: (Targa, Marca, Modello, Anno, Assicurazione, Note, Stato)
    ...
    def mostra_dettagli(self, tipo, dati):
        ...
        # Scegliamo le etichette in base al tipo
        campi = self.campi_d if tipo == "Driver" else self.campi_v

        # Creiamo una lista di label per ogni campo
