...
class SistemaGestionaleFlotta:
    ...
    def aggiorna_titolo_finestra(self):
        name_db = os.path.basename(self.db.db_path)
        self.root.title(f"Gestione Flotta Pro - by Viorel  |  📂 Database Attivo: {name_db}")
    ...
    def esporta_veicoli_csv(self):
        ...
        if path:
            ...
            # Mappa per capire se il driver assegnato oggi è assente
    ...
    def crea_interfaccia(self):
        ...
        self.tab_p = self.tabs.add("       📅 PRESENZE          ")  # SCHEDA AGGIUNTA

        self.setup_tab_anagrafica(perm)
        self.setup_tab_veicoli(perm)
        self.setup_tab_presenze(perm)  # SETUP AGGIUNTO
    ...
    # INTERFACCIA GESTIONE PRESENZE AGGIUNTA
    def setup_tab_presenze(self, perm):
        tb = ctk.CTkFrame(self.tab_p, fg_color="transparent")
        tb.pack(fill="x", pady=10)

        ctk.CTkLabel(tb, text="Data (GG/MM/AAAA):", font=("Helvetica", 12, "bold")).pack(side="left", padx=5)
        self.entry_data_presenze = ctk.CTkEntry(tb, width=120)
        self.entry_data_presenze.insert(0, datetime.now().strftime("%d/%m/%Y"))
        self.entry_data_presenze.pack(side="left", padx=5)

        ctk.CTkButton(tb, text="🔄 CAMBIA DATA / AGGIORNA", fg_color="#34495e", command=self.aggiorna_presenze).pack(
            side="left", padx=5)

        # Pulsanti rapidi per assegnare stato di presenza
        ctk.CTkButton(tb, text="✅ PRESENTE", fg_color="#2ecc71", text_color="white",
                      command=lambda: self.salva_stato_presenza("Presente"), state=perm).pack(side="left", padx=5)
        ctk.CTkButton(tb, text="🌴 FERIE", fg_color="#e67e22", command=lambda: self.salva_stato_presenza("Ferie"),
                      state=perm).pack(side="left", padx=5)
        ctk.CTkButton(tb, text="🤒 MALATTIA", fg_color="#9b59b6", command=lambda: self.salva_stato_presenza("Malattia"),
                      state=perm).pack(side="left", padx=5)
        ctk.CTkButton(tb, text="📄 PERMESSO", fg_color="#3498db", command=lambda: self.salva_stato_presenza("Permesso"),
                      state=perm).pack(side="left", padx=5)
        ctk.CTkButton(tb, text="❌ ASSENTE INGIUSTIFICATO", fg_color="#e74c3c",
    ...
    def crea_tree(self, parent, colonne):
        ...
        tree.tag_configure('assente_tag', background='#d35400')  # Tag visivo per assenti
    ...
    def aggiorna_dati(self):
        ...
        # Rilevamento presenze ad oggi per determinare lo stato reale del mezzo
        oggi_str = datetime.now().strftime("%d/%m/%Y")
        self.db.cursor.execute("SELECT id_driver, stato FROM presenze WHERE data=?", (oggi_str,))
        ...
        for row in self.db.cursor.fetchall():
            ...
            stato_presenza = mappa_presenze_oggi.get(id_d, "Presente")  # Se non specificato, si assume presente

            if stato_presenza != "Presente":
                # Se il driver è assente, NON occupa il mezzo nella visualizzazione logica
                driver_targa_map[targa] = f"Disponibile ({stato_presenza})"
            else:
        ...
        for v in self.db.cursor.fetchall():
            ...
            # Se attivo il filtro "Solo Liberi", mostro sia i disponibili totali che quelli liberi per assenza driver
    ...
    # LOGICA AGGIORNAMENTO TABELLA VISIVA DELLE PRESENZE
    def aggiorna_presenze(self):
        if not hasattr(self, 'tree_p'): return
        for i in self.tree_p.get_children(): self.tree_p.delete(i)

        data_sel = self.entry_data_presenze.get().strip()
        if not re.match(r"^\d{2}/\d{2}/\d{4}$", data_sel):
            return  # Evita crash se l'utente sta digitando la data
        ...
        for d in self.db.cursor.fetchall():
            id_d = d[0]
            stato_attuale = stati_salvati.get(id_d, "Presente")  # Default iniziale
    ...
    # SALVATAGGIO DELLA PRESENZA SUL DATABASE
    def salva_stato_presenza(self, nuovo_stato):
        ...
        data_sel = self.entry_data_presenze.get().strip()
        if not re.match(r"^\d{2}/\d{2}/\d{4}$", data_sel):
            messagebox.showerror("Errore Data", "Formato data non valido. Usa GG/MM/AAAA")
            return
        ...
        for item in sel:
            ...
            # Inserisce o aggiorna lo stato per quel giorno e quel driver
        ...
        self.aggiorna_dati()  # Aggiorna tutto il sistema (compreso lo stato libero/occupato dei veicoli)
    ...
    def mostra_dettagli(self, tipo, dati):
        ...
        for i, campo in enumerate(campi):
            ...
            ctk.CTkLabel(fr, text=str(valore), font=("Helvetica", 12)).pack(side="right")
