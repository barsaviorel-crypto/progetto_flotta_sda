...
#        self.root.minsize(1024, 600)
    ...
    def elimina_driver(self):
        # Recupera tutti gli elementi selezionati nella tabella
        selezionati = self.tree_d.selection()

        if not selezionati:
            messagebox.showwarning("Alt", "Seleziona almeno un driver da eliminare.")
            return

        # Determina il messaggio in base a quanti driver sono stati scelti
        if len(selezionati) == 1:
            valori = self.tree_d.item(selezionati[0])['values']
            messaggio = f"Eliminare il driver selezionato: {valori[1]} {valori[2]}?"
        else:
            messaggio = f"Sei sicuro di voler eliminare i {len(selezionati)} driver selezionati?"

        # Chiede la conferma all'utente
        if messagebox.askyesno("Conferma Eliminazione Multipla", messaggio):
            for item in selezionati:
                id_driver = self.tree_d.item(item)['values'][0]

                # Opzionale: Libera il veicolo associato al driver prima di eliminarlo
                self.db.cursor.execute("UPDATE veicoli SET note='' WHERE targa = (SELECT targa FROM driver WHERE id=?)",
                                       (id_driver,))

                # Elimina il driver dal database
                self.db.cursor.execute("DELETE FROM driver WHERE id=?", (id_driver,))
