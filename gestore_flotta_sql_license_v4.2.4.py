...
def get_hwid():
    ...
    # 1. HARDWARE VIA REGISTRO (Istantaneo - Usiamo DEVNULL nativo di Python per gli errori)
    try:
    ...
    # 2. HARDWARE VIA POWERSHELL (Più lento ma sicuro - Usiamo DEVNULL nativo di Python)
    try:
    ...
    # 3. ULTIMA SPIAGGIA: SOFTWARE MachineGuid (Se l'hardware è totalmente bloccato)
    try:
...
class SistemaGestionaleFlotta:
    ...
    def mostra_login(self):
        ...
        # 👇 FUNZIONE PER GESTIRE LA CHIUSURA DALLA "X" CON CONFERMA
        def on_login_close():
            winsound.MessageBeep(winsound.MB_OK)  # Beep di domanda/conferma
            if messagebox.askyesno("Esci", "Vuoi davvero uscire dal programma?", parent=win):
                win.destroy()  # Chiude il pannello di login
                self.root.destroy()  # Abbatte l'intero processo dell'applicazione
        # Intercettiamo il click sulla "X" di questa finestra
        ...
        def login():
            ...
            if not u or not p_raw:
                winsound.MessageBeep(winsound.MB_ICONHAND)  # Beep di errore brutale
            ...
            if count == 0:
                ...
                self.db.conn.commit()
                winsound.MessageBeep(winsound.MB_OK)  # Beep di notifica standard successo
            ...
            if res:
                ...
                winsound.MessageBeep(winsound.MB_ICONHAND)  # Beep di errore
    ...
    def valida_data_stringa(self, data_str):
        """Verifica se la stringa è nel formato GG/MM/AAAA e se corrisponde a una data valida."""
        # Controlla il formato tramite espressione regolare
        if not re.match(r"^\d{2}/\d{2}/\d{4}$", data_str):
            return False
        try:
            # Tenta di convertire la stringa in un oggetto datetime reale
            datetime.strptime(data_str, "%d/%m/%Y")
            return True
        except ValueError:
            # Se fallisce (es. 31/02/2026 o mese 13), la data non è valida
            return False
    ...
    def form_veicolo(self, dati=None):
        ...
        def salva():
            ...
            targa_nuova = d[0]
            data_assicurazione = d[4]  # <--- Campo Assicurazione

            if not targa_nuova:
                messagebox.showerror("Errore", "La Targa è obbligatoria!", parent=win)
                return

             # 👇 NUOVO CONTROLLO DATA ASSICURAZIONE
            if data_assicurazione:  # Applica il controllo solo se il campo non è vuoto
                if not self.valida_data_stringa(data_assicurazione):
                    winsound.MessageBeep(winsound.MB_ICONHAND)
                    messagebox.showerror("Errore Data","La data di Assicurazione deve essere nel formato valido GG/MM/AAAA\n(Es. 15/09/2026)",
                                             parent=win)
                    return
    ...
    def form_driver(self, dati=None):
        ...
        def salva():
            ...
            if nascita:  # Applica il controllo solo se il campo non è vuoto
    ...
    def on_closing(self):
        self.salva_configurazione_completa()
        if self.modificato:
            winsound.MessageBeep(winsound.MB_OK)  # Beep di domanda
            ...
            winsound.MessageBeep(winsound.MB_ICONASTERISK)  # Beep di domanda
