...
def verifica_licenza():
    ...
    if id_autorizzato == get_hwid():
        ...
        messagebox.showerror("Errore",
                             f"Licenza non valida per questo computer.ID richiesto: {get_hwid()}")
...
class SistemaGestionaleFlotta:
    ...
    # --- POPUP DETTAGLI AVANZATI (INVIO / DOPPIO CLICK) ---
    def popup_dettagli_driver(self, event=None):
        ...
        win.title(f"Scheda Driver: {valori[1]} {valori[2]}")
        win.geometry("500x520")
        win.attributes("-topmost", True)
        win.grab_set()

        ctk.CTkLabel(win, text="SCHEDA DETTAGLI DRIVER", font=("Helvetica", 18, "bold"), text_color="#3498db").pack(
            pady=15)

        fr = ctk.CTkFrame(win, fg_color="#2b2b2b")
        fr.pack(fill="both", expand=True, padx=20, pady=10)

        colonne_complete = ["ID"] + self.campi_d
        for idx, campo in enumerate(colonne_complete):
            fr_riga = ctk.CTkFrame(fr, fg_color="transparent")
            fr_riga.pack(fill="x", padx=15, pady=6)

            lbl_chiave = ctk.CTkLabel(fr_riga, text=f"{campo.upper()}:", font=("Helvetica", 12, "bold"), width=120,
                                      anchor="w")
            lbl_chiave.pack(side="left")

            val_testo = str(valori[idx]) if valori[idx] is not None else ""
            lbl_valore = ctk.CTkLabel(fr_riga, text=val_testo, font=("Helvetica", 13), text_color="#e0e0e0", anchor="w")
            lbl_valore.pack(side="left", fill="x", expand=True)

        ctk.CTkButton(win, text="CHIUDI", fg_color="#34495e", command=win.destroy).pack(pady=15)

    def popup_dettagli_veicolo(self, event=None):
        sel = self.tree_v.selection()
        if not sel:
            return
        valori = self.tree_v.item(sel[0])['values']

        win = ctk.CTkToplevel(self.root)
        win.title(f"Scheda Veicolo: {valori[0]}")
        win.geometry("500x420")
        win.attributes("-topmost", True)
        win.grab_set()

        ctk.CTkLabel(win, text="SCHEDA DETTAGLI VEICOLO", font=("Helvetica", 18, "bold"), text_color="#2ecc71").pack(
            pady=15)

        fr = ctk.CTkFrame(win, fg_color="#2b2b2b")
        fr.pack(fill="both", expand=True, padx=20, pady=10)

        for idx, campo in enumerate(self.campi_v):
            fr_riga = ctk.CTkFrame(fr, fg_color="transparent")
            fr_riga.pack(fill="x", padx=15, pady=6)

            lbl_chiave = ctk.CTkLabel(fr_riga, text=f"{campo.upper()}:", font=("Helvetica", 12, "bold"), width=130,
                                      anchor="w")
            lbl_chiave.pack(side="left")
    ...
    def setup_tab_anagrafica(self, perm):
        ...
        # Collegamento eventi doppio click e invio per i Driver
        self.tree_d.bind("<Double-1>", self.popup_dettagli_driver)
        self.tree_d.bind("<Return>", self.popup_dettagli_driver)
    ...
    def setup_tab_veicoli(self, perm):
        ...
        # Collegamento eventi doppio click e invio per i Veicoli
        self.tree_v.bind("<Double-1>", self.popup_dettagli_veicolo)
        self.tree_v.bind("<Return>", self.popup_dettagli_veicolo)
    ...
    def aggiorna_dati(self):
        ...
        self.db.cursor.execute("SELECT targa, nome, cognome FROM driver WHERE targa != ''")
        driver_targa_map = {row[0]: f"{row[1]} {row[2]}" for row in self.db.cursor.fetchall()}

        oggi = datetime.now()
        self.db.cursor.execute("SELECT * FROM veicoli")
        for v in self.db.cursor.fetchall():
            stato = driver_targa_map.get(v[0], "Disponibile")
